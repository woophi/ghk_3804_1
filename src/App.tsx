...
const texts = [
  {
    title: 'Вы посадили доходное дерево',
    subtitle: (
      <>
  ...
  {
    title: 'Инвестируйте по чуть-чуть',
    subtitle: (
      <>
        Важны не суммы,
        <br />а регулярные пополнения
      </>
    ),
    step: 1,
  },
  {
    title: 'Пополняйте каждый месяц',
    subtitle: (
      <>
        Не обязательно слишком часто,
        <br />
  ...
  },
  {
    ...
    subtitle: (
      <>
        Даже самые маленькие, но регулярные
        <br />
        пополнения, помогают дереву расти
      </>
    ),
    step: 3,
  },
  {
    ...
    subtitle: (
      <>
        Не думайте о пополнении — настройте
        <br />
        минимальную сумму и просто ждите
      </>
    ),
    step: 7,
  },
        ...
        >
          {options.map((o, index) => (
            <SwiperSlide
              ...
              className={appSt.swSlide({ selected: index === step })}
              key={o.title}
            >
              ...
              <img src={o.icon} alt={o.title} width={24} height={24} />
            </SwiperSlide>
          ))}
        </Swiper>
      </div>
